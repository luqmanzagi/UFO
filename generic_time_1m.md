...
**OBJECTIVE:** Determine if login is required, perform **STOCHASTIC (RANDOM) EXPLORATION** for exactly 60 seconds using your internal clock, and then forcefully close the application.
...
4. **INTERNAL CLOCK SYNC:** You must monitor the `elapsed_time` variable from your local UFO clock. 
   ...
   - **Constraint:** Once `elapsed_time >= 60`, you MUST stop exploration and proceed to PHASE 4.
...
**Goal:** Simulate random user behavior until the clock hits 60 seconds.



1. **Pre-Action Clock Check:** Read `elapsed_time`. If `>= 60`, Transition to -> **PHASE 4**.
...
`[PHASE]:` <Phase ID>
`[TIMER]:` <Current elapsed_time> / 60s

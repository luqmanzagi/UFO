...
# ---- helpers -----------------------------------------------------------------
function Set-Normalize-Name([string]$s) {
...
function Get-Aliases([string]$name) {
  ...
  $aliases.Add((Set-Normalize-Name $name)) | Out-Null
  ...
  foreach ($a in $snapshot) {
    $aliases.Add((Set-Normalize-Name $a)) | Out-Null
  }

  # common shortener for "X: subtitle"
  if ($name -match '^(.+?):\s') {
    $aliases.Add($Matches[1]) | Out-Null
    $aliases.Add((Set-Normalize-Name $Matches[1])) | Out-Null
  }
...
function Find-InstalledAppName([string]$targetName) {
  ...
  foreach ($sa in $startApps) {
    $norm = Set-Normalize-Name $sa.Name
  ...
  $allNorms = $index.Keys
  foreach ($cand in $candidates) {
    $normCand = Set-Normalize-Name $cand
...
function Invoke-StartMenuLaunch ([string]$text) {
  try {
...
foreach ($rawApp in $apps) {
  ...
  if ($launched) {
    ...
    foreach ($cand in ($aliasSet | Sort-Object Length)) {
      if (Invoke-StartMenuLaunch $cand) { $displayName = $cand; break }
    }
  }
